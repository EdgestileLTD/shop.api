...
class Product extends Base
{
	...
	{
		$idsProduct = empty($this->input["ids"]) ? array($this->input["id"]) : $this->input["ids"];
		if (!$idsProduct)
			return;
	...
	private function getSpecificationByName($specification)
	{
	...
	{
		$idsProducts = empty($this->input["ids"]) ? array($this->input["id"]) : $this->input["ids"];
		if (!$idsProducts)
			return;
