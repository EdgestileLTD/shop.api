...
function convertFields($str)
{
    ...
    $str = str_replace('[idProvider]', 'spp.id_provider', $str);
...
$u->select('sp.*, sg.name as namegroup, sdl.discount_id is_discount, SUM(spp.count) count, MIN(spp.price) minPrice,
...
if (!empty($searchStr)) {
    if (strpos($searchStr, '?') === 0) {
        ...
        foreach ($searchArr as $searchItem) {
            ...
            $search .= "(`sp`.`name` like '%$searchItem%' OR `sg`.`name` like '%$searchItem%'
...
foreach ($objects as $item) {
    ...
    $product['price'] = (real)$item['minPrice'];
    $product['pricePurchase'] = (real)$item['price_purchase'];
    $product['currency'] = $item['curr'];
    $product['providersDisplay'] = $item['providersDisplay'];
    ...
    if ($product['imageFile']) {
        ...
        } else $product['imageUrl'] = $product['imageFile'];
    }
