...
function saveSpecifications($idsProducts, $specifications)
{
    ...
    foreach ($specifications as $specification) {
        ...
        if (($specification->valueType == "L" || $specification->valueType == "CL") && ($idValue == 'null') &&
...
function saveProviders($idsProducts, $providers)
{
    foreach ($idsProducts as $idProduct) {
        $u = new seTable('shop_providers_price', 'sp');
        $u->select('sp.*');
        ...
        foreach ($items as $item) {
            foreach ($providers as $provider)
                if ($provider->id == $item["id_provider"]) {
                    $idsUpdated[] = $provider->id;
                    if ($item['count'] != $provider->count || $item['price'] != $provider->price) {
                        setField(false, $u, $provider->count, 'count');
                        setField(false, $u, $provider->price, 'price');
                        $u->where('id=?', $item['id']);
                        $u->save();
                    }
                }
        ...
        $u->where('id_price=?', $idProduct);
        if ($idsUpdated)
            $u->andwhere('NOT id_provider IN (?)', implode(",", $idsUpdated));
        $u->deletelist();
        $data = array();
        foreach ($providers as $provider)
            if (!in_array($provider->id, $idsUpdated)) {
                $data[] = array('id_price' => $idProduct, 'id_provider' => $provider->id,
                    'count' => $provider->count, 'price' => $provider->price);
            }
        if (!empty($data))
            se_db_InsertList('shop_providers_price', $data);
    }
}
...
if ($isNew || !empty($ids)) {
    ...
    if (isset($json->brand)) {
        ...
        else $isUpdated |= setField($isNew, $u, null, 'id_brand');
    }
    ...
    if ($ids && isset($json->specifications))
        saveSpecifications($ids, $json->specifications);
    ...
    if ($ids && isset($json->providers))
        saveProviders($ids, $json->providers);
