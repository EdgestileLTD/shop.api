    ...
    foreach ($objects as $item) {
        ...
        if ($image['imageFile']) {
            if (strpos($image['imageFile'], "://") === false) {
                $image['imageUrl'] = 'http://' . $json->hostname . "/images/rus/shopprice/" . $image['imageFile'];
                $image['imageUrlPreview'] = "http://{$json->hostname}/lib/image.php?size=64&img=images/rus/shopprice/" . $image['imageFile'];
            } else {
...
function getModifications($ids, &$product)
{
    ...
    $u->innerjoin('shop_group_feature sgf', 'sgf.id_group = sm.id_mod_group');
    ...
    $u->innerjoin('shop_group_feature sgf', 'sgf.id_feature = sfvl.id_feature');
                ...
                foreach ($images as $image) {
                    ...
                    if ($value['imageFile']) {
                        if (strpos($value['imageFile'], "://") === false) {
                            $value['imageUrl'] = 'http://' . $json->hostname . "/images/rus/shopprice/" . $value['imageFile'];
                            $value['imageUrlPreview'] = "http://{$json->hostname}/lib/image.php?size=64&img=images/rus/shopprice/" . $value['imageFile'];
                        } else {
...
function getProviders($id, &$product)
{
    $u = new seTable('shop_providers', 'sp');
    $u->select('sp.*, spp.count, spp.price');
    $u->innerjoin('shop_providers_price spp', 'spp.id_provider = sp.id');
    $u->where('spp.id_price = ?', $id);
    $objects = $u->getList();
    foreach ($objects as $item) {
        $provider = null;
        $provider['id'] = $item['id'];
        $provider['provider'] = $item['name'];
        $provider['price'] = (real)$item['price'];
        $provider['count'] = (real)$item['count'];
...
$u->select('sp.*, sg.id idGroup, sb.name AS nameBrand, sg.name AS nameGroup, sg.id_modification_group_def,
           SUM(spp.count) count,  MIN(spp.price) minPrice,           
           GROUP_CONCAT(DISTINCT spr.name ORDER BY spr.name ASC SEPARATOR ",") providersDisplay');
$u->leftjoin('shop_brand sb', 'sb.id = sp.id_brand');
$u->leftjoin('shop_providers_price spp', 'spp.id_price = sp.id');
$u->leftjoin('shop_providers spr', 'spp.id_provider=spr.id');
if (CORE_VERSION == "5.3") {
...
if ($u->id) {
    ...
    $product['providersDisplay'] = $u->providersDisplay;
    $product['count'] = (float)$u->count;
...
if (se_db_error()) {
    $status['status'] = 'error';
    $status['errortext'] = 'Не удаётся получить информацию о товаре!';
} else {
    $status['status'] = 'ok';
    $status['data'] = $data;
}
