...
function convertFields($str)
{
    ...
    $str = str_replace('idCoupon', 'id_coupon', $str);
    $str = str_replace('[idProvider]', 'so.id_provider', $str);
    return $str;
}
...
$u->select('so.*, sto.nameitem, CONCAT_WS(" ", p.last_name, p.first_name, p.sec_name) as customer, p.phone as customerPhone, p.email as customerEmail,
            ...
            spp.name_payment paymentType, ss.id order_status, spr.name provider');
$u->innerjoin('person p', 'p.id=so.id_author');
$u->innerjoin('shop_tovarorder sto', 'sto.id_order=so.id');
$u->leftjoin('shop_status ss', 'ss.id=so.id_status');
...
$u->leftjoin('shop_providers spr', 'spr.id=so.id_provider');
...
if ($count > 0) {
    foreach ($result as $item) {
        ...
        $order['statusOrder'] = $item['order_status'];
