...
if ($headers['Origin'] || $headers['origin']) {
    $url = $headers['Origin'] ? parse_url($headers['Origin']) : parse_url($headers['origin']);
    if ($url) {
...
if ($json->token) {
    ...
    if (IS_EXT)
        ...
        } else $dbConfig = empty($folderShop) ? PATH_ROOT . $json->hostname . '/public_html/system/config_db.php' :
            PATH_ROOT . $json->hostname . "/projects/{$folderShop}/config/config_db.php";
    }
    if (file_exists($dbConfig)) {
        include $dbConfig;
